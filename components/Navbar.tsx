...
export default function Navbar({ setIsSideBarVisible }: { setIsSideBarVisible: Dispatch<SetStateAction<boolean>> }) {
  ...
  return (
    <div className="flex flex-col h-48 static sm:flex-row text-white sm:h-20 font-Open py-5 z-20 px-3 sm:px-5 sm:pt-10 justify-between  align-middle items-center w-full">
      ...
      <div className="flex sm:items-center justify-between w-full sm:justify-end sm:w-2/12 px-2 mt-5 sm:mt-0">
        <div className="flex justify-between w-full flex-row sm:flex-row items-end sm:items-center">
          <div className="flex flex-row-reverse sm:flex-row items-center">
            ...
            <button
              onClick={handleClick}
              className="mr-3 rounded-md w-24 h-9 sm:mx-10 text-white bg-buttonBlue focus:outline-none shadow-inputShadow ">
              <a href="/">Logout</a>
            </button>
          </div>
