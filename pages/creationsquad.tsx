...
export default function CreationSquad(): JSX.Element {
  ...
  return (
    <div className="flex h-screen w-full mt-5 mx-4 flex-col  text-white  overflow-y-auto  ">
      ...
      <form className=" font-Open  mt-3 flex flex-col" onSubmit={handleSubmit(onSubmit)}>
        ...
        <select className="rounded-md text-white focus:ouline-none bg-grayinput bg-opacity-30 shadow-inputShadow p-2" {...register('business', {})}>
          {!BusinessLoading &&
            BusinessesDatas.BusinessSector.map((business: { id: string; name: string }) => {
              return (
